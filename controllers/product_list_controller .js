  ...
  if (detailCodeId) {
    ...
    try {
      const subcategoryProductList =
        await productListService.subcategoryProductList(
          groupCodeId,
          detailCodeId,
          limit,
          offset
      ...
      res.status(200).json({
        productList: subcategoryProductList,
        message: "success_subcategoryProductList",
      });
    } catch (err) {
      console.log(err);
    ...
    try {
      const groupProductList = await productListService.groupProductList(
        groupCodeId,
        limit,
        offset
      );
      res.status(200).json({
        productList: groupProductList,
        message: "success_groupProductList",
      });
    } catch (err) {
      console.log(err);
      res.status(500).json(err);
...
const productSearch = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ productSearchList: result, message: "readProductSearchList" });
  } catch (err) {
    console.log(err);
    res.status(500).json(err);
  }
...
const recommendProductList = async (req, res) => {
  try {
    ...
    res.status(200).json({
      recommendProductList: result,
      message: "readRecommendProductList",
    });
  } catch (err) {
    console.log(err);
    res.status(500).json(err);
...
const newProductList = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ newProductList: result, message: "readNewProductList" });
  } catch (err) {
    console.log(err);
    res.status(500).json(err);
  }
